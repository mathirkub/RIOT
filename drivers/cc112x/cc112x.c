...
int cc112x_setup(cc112x_t *dev, const cc112x_params_t *params)
{
    ...
    /* Settign CCA signal on GPIO3 */
    cc112x_write_register(dev, CC112X_IOCFG3, 15);
...
{
    LOG_INFO("%s:%s:%u setting address %u\n", RIOT_FILE_RELATIVE, __func__,
...
{
    LOG_DEBUG("cc112x: switching to powerdown mode\n");
...
int16_t cc112x_set_channel(cc112x_t *dev, uint8_t channr)
{
    ...
    cc112x_write_register(dev, CC112X_FREQ0, (uint8_t)(regval & 0xff));
    cc112x_write_register(dev, CC112X_FREQ1, (uint8_t)((regval & 0xff00) >> 8));
    cc112x_write_register(dev, CC112X_FREQ2, (uint8_t)((regval & 0xff0000) >> 16));
...
void cc112x_write_register(cc112x_t *dev, uint16_t r, uint8_t value)
{
...
int cc112x_rd_set_mode(cc112x_t *dev, cc112x_radio_mode_t mode)
{
    ...
    switch(mode) {
    case RADIO_MODE_ON:
        LOG_DEBUG("%s:%s:%u: cc112x: switching to RX mode\n", RIOT_FILE_RELATIVE, __func__, __LINE__);
...
uint8_t cc112x_get_RSSI(cc112x_t *dev){
    return cc112x_read_reg(dev, CC112X_RSSI1);
}
