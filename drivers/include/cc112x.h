...
/**
 * @brief Performs simple test (FIFO, states etc.)
 *
 * @params[in] dev      device struct to test
 *
 * @return  suceed or failed
 */

bool cc112x_test(cc112x_t *dev);

/**
 * @brief Set the transceiver to sleep mode.
 */

void cc112x_sleep(cc112x_t *dev);

/**
 * @brief Wakes up the transceiver to RX mode.
 */
